...
class CamConfig:
    def __init__(self):
        ...
        # self.cap = cv2.VideoCapture("C:\\Users\\BoySong\\Pictures\\Camera Roll\\test.mp4")
    ...
    def show_pic(self):
        ...
        if success:
            #保存图片
            folder_name = img_path + "/native"
            os.makedirs(folder_name, exist_ok=True)
            filename = createRandomString(8)
            cv2.imwrite(folder_name +"/" + filename + '.jpg', frame)
            # 检测
            # 将摄像头读到的frame传入检测函数myframe.frametest()
            ret,frame = myframe.frametest(frame, img_name=filename + '.jpg')
            ...
            for i in lab:
                if i in ("phone", "smoke", "drink"):
                    v = Voice(id=1, name='lulu', languages='chinese', age=10, gender='男')
                    engine.setProperty('voice', v)
                    engine.say('滴')
                    engine.runAndWait()  # 朗读一次
                if(i == "phone"):
                   # pla.playsound("F:\workspace\PyCharm\Yolov5-deepsort-driverDistracted-driving-behavior-detection-1.0\\test.MP3")
            ...
            global num
            num = num + 1
            print("图片"+str(num)+"：" + filename + ",眼睛开合程度，阀值，计数器,总数 :", (eye, EYE_AR_THRESH, COUNTER, TOTAL))
...
def createRandomString(len):
    ...
    #     raw += chr(seed);
    #     i += 1
    raw = str(int(round(time.time()*10000)))
    raw = raw[4:]
