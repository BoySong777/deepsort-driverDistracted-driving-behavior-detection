...
def detfatigue(frame):
    ...
    for rect in rects:
        ...
        # 标注识别结果
        # 使用cv2.convexHull获得轮廓位置，使用drawContours画出轮廓位置进行画图操作
...
def mydetection():
    #原始文件路径
    root_path = "F:/data_set/dangerous_driving_behavior/test/"
    dir = root_path + "train" + "/"
    folder_name = root_path + "train2" + "/"
    os.makedirs(folder_name, exist_ok=True)

    count = 0
    for root, dir, files in os.walk(dir):
        for file in files:
            srcImg = cv2.imread(root_path + "train" + "/" + str(file))
            frame, eyear, mouthar = detfatigue(srcImg)
            cv2.imwrite(folder_name + str(file), frame)
            count += 1
        print(count)
